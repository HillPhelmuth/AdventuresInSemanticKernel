...
{
    public partial class SemKernelSamples : ComponentBase
    {
        [Inject]
        ...
        private static readonly string s_codeFilesPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) ?? Directory.GetCurrentDirectory(), "Data", "CodeFiles");
        private List<CodeFile> _exampleCodeFiles = new();
        ...
        {
            _exampleCodeFiles = Directory.GetFiles(s_codeFilesPath).Select(x => new CodeFile { Name = Path.GetFileNameWithoutExtension(x), Code = File.ReadAllText(x) }).ToList();
        ...
        private async void Execute(bool compileAndRun)
        {
            ...
            if (compileAndRun)
            {
                ...
                var executableCode = $"{CodeSampleTemplate.UsingStatements}\n\n{code}";
