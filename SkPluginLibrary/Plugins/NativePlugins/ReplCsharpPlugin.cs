...
public class ReplCsharpPlugin
{
    ...
    public ReplCsharpPlugin(Kernel kernel)
    {
        ...
        if (codingPlugin.TryGetFunction("CodeCSharp", out var codeFunction))
            _generateCodeFunction = codeFunction;
        else
        {
            var file = Directory.GetFiles(Path.Combine(RepoFiles.PathToYamlPlugins, "CodingPlugin"), "*.yaml").FirstOrDefault(x => x.Contains("CodeCSharp"));
            if (file != null)
            {
                _generateCodeFunction = kernel.CreateFunctionFromPromptYaml(File.ReadAllText(file));
            }
            else
            {
                throw new Exception("Could not find CodeCSharp function in CodingPlugin");
            }

        }
        if (codingPlugin.TryGetFunction("CSharpScript", out var scriptFunction))
            _generateScriptFunction = scriptFunction;
        else
        {
            var file = Directory.GetFiles(Path.Combine(RepoFiles.PathToYamlPlugins, "CodingPlugin"), "*.yaml").FirstOrDefault(x => x.Contains("CSharpScript"));
            if (file != null)
            {
                _generateScriptFunction = kernel.CreateFunctionFromPromptYaml(File.ReadAllText(file));
            }
            else
            {
                throw new Exception("Could not find CodeCSharp function in CodingPlugin");
